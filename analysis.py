...
def calculate_drop_rate(sent_dfs, rcvd_dfs):
    ...
    for sent_df in sent_dfs:
        ...
        sent_uuids = set(sent_df["uuid"])

        total_sent = len(sent_uuids)
        # total_received = 0

        for rcvd_df in rcvd_dfs:
            recipient = rcvd_df["recipient"].iloc[0]
            if recipient != sender:
                received_uuids = set(rcvd_df["uuid"])
                total_received = len(sent_uuids.intersection(received_uuids))

                # print(
                #     f"Drop rate for {sender}->{recipient}: {1 - total_received / total_sent}"
                # )
                total_dropped = total_sent - total_received
                drop_rate = total_dropped / total_sent if total_sent > 0 else 0
    ...
    return pd.DataFrame(drop_rate_data).sort_values(["sender", "recipient"])


def analysis_orchestrator(logs_path, plot_graphs):
    sent_csv_files, rcvd_csv_files = get_files(logs_path)
    sent_dfs_arr, rcvd_dfs_arr = generate_dataframes(
        sent_csv_files
    ), generate_dataframes(rcvd_csv_files)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plot", action="store_true", help="Plot the latency graphs (default: False)"
    )
